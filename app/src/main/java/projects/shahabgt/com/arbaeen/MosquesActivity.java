...
import com.example.jonathan.arbaeen.classes.LocationInfo;
import com.example.jonathan.arbaeen.classes.Place;
import com.example.jonathan.arbaeen.classes.PlacesService;
...
//        getSupportActionBar().setTitle(R.string.action_mosque);
    ...
    public void onBackPressed() {
        startActivity(new Intent(MosquesActivity.this,IntroActivity.class));
        this.finish();
    }
...
//        LatLng position = new LatLng(52.011138, 4.357177);
    ...
    class Places extends AsyncTask<LatLng, Void, List<Place>> {
        ...
        protected List<Place> doInBackground(LatLng... latLngs) {
            SharedPreferences sp = sp= MosquesActivity.this.getSharedPreferences("location",0);
            float lat = Float.parseFloat(sp.getString("lat",getResources().getString(R.string.default_lat)));
            float lon = Float.parseFloat(sp.getString("lon",getResources().getString(R.string.default_lon)));
            String name =sp.getString("loc","Tehran");
            float timezone = (float) (TimeZone.getTimeZone(Time.getCurrentTimezone()).getOffset(System.currentTimeMillis()));
            LocationInfo locationInfo = new LocationInfo(lat,lon,name,name,"IR",name,"AS",364,1,4,1,timezone,"ایران");
            PlacesService service = new PlacesService("AIzaSyCQm0hVFIduNtHFXtrcFVYQr-kQni29kbU");
